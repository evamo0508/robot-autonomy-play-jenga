...
def knock_down(task):
    ...
    obs = move(0.3,-0.4,0.9,roll,pitch,yaw,task)
    obs = move(0.1,0.3,0.75,roll,pitch,yaw,task)
    obs = move(0.2,-0.2,0.75,roll,pitch,yaw,task)
    
    #clear
    roll,pitch,yaw = np.pi,0,np.pi*1/2
    obs = move(0.2,-0.3,0.75,roll,pitch,yaw,task)
    obs = move(0.2,0.1,0.75,roll,pitch,yaw,task)  
    obs = move(0.1,0.1,0.75,roll,pitch,yaw,task)
    obs = move(0.1,-0.1,0.75,roll,pitch,yaw,task)
    obs = move(0.1,-0.1,1.0,roll,pitch,yaw,task)
    return obs
...
def find_cuboid_2_grasp(obs, obj_poses, visited, task):
    ...
    move(wptPick[0],wptPick[1],wptPick[2]+0.1,orientation[0],orientation[1],orientation[2],task)
